<template>
  <div class="events-page">
    ...
    <!-- Filtro -->
    <el-drawer v-model="drawer" title="Filtrar por" :direction="'btt'" :before-close="handleClose" size="65%">
    ...
    <el-tabs v-model="activeTab" @tab-change="handleTabChange">
      ...
      <el-tab-pane label="Encerrados" name="ended">
        ...
        <div v-if="!loadedOldEvents">
          <el-button type="primary" icon="el-icon-loading" loading class="loading"></el-button>
        </div>
        <template v-else>
...
const cities = ref<{ [key: string]: string[] }>({}); // Modificação para usar um hash map para cidades

const optionsEstados = ref([]); 
const optionsCidades = ref([]); 

const selectedState = ref(''); // Variável reativa para o estado selecionado
...
const getEstados = () => {
  ...
  console.log('Estados:', optionsEstados.value);
};

const getCidades = () => {
  const cidades = {}; // Usar um objeto para mapear estados para listas de cidades
  if (currentEvents.value) {
    currentEvents.value.forEach(event => {
      if (event.location && event.location.state && event.location.city) {
        const estado = event.location.state;
        const cidade = event.location.city;
        if (!cidades[estado]) {
          cidades[estado] = [];
        }
        cidades[estado].push(cidade);
      }
    });
  }
  cities.value = cidades;
  console.log('Cidades por Estado:', cities.value);
};

// Função para atualizar as cidades quando um estado é selecionado
const updateCidadesForSelectedState = (state) => {
  selectedCity.value = ''; // Resetar a cidade selecionada
  if (cities.value[state]) {
    optionsCidades.value = cities.value[state].map(cidade => ({
      value: cidade,
      label: cidade
    }));
  } else {
    optionsCidades.value = [];
  }
  console.log('Cidades para o estado selecionado:', optionsCidades.value);
};

watch(selectedState, (newState) => {
  console.log('Estado mudou para:', newState); // Log da mudança de estado
  updateCidadesForSelectedState(newState);
});
...
const selectState = (state) => {
  ...
  console.log('Estado selecionado pelo usuário:', state); // Log do estado selecionado pelo usuário
};
...
const filterByLocation = () => {
  ...
  filteredOngoingEvents.value = currentEvents.value.filter(event => {
    ...
    const matchesState = !selectedStateValue || eventState === selectedStateValue;
    const matchesCity = !selectedCityValue || eventCity === selectedCityValue;
...
const applyLocationFilter = () => {
  ...
  selectedLocationLabel.value = selectedCityValue ? `${selectedCityValue}, ${selectedStateValue}` : selectedStateValue;
